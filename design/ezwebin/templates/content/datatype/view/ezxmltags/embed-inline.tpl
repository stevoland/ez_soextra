...
*}{if or(is_set($#use_inline_styles), is_set($use_inline_styles))}{*
	*}{set $embed_style=concat($embed_style, cond($object_parameters.align|eq('right'), 'clear:right;float:right;margin-left:6px;', 'clear:left;float:left;margin-right:6px;') ) }{*
*}{else}{*
...
*}{if and(is_set($border_size), $border_size|trim)}{set $embed_style=concat($embed_style, 'border-width:', $border_size|wash, '; ') }{set $border_size=''}{/if}{/if}{content_view_gui view=$view link_parameters=$link_parameters object_parameters=$object_parameters content_object=$object classification=$classification in_oe=cond(is_set($in_oe),true()) inline=true() inner_style=$embed_style css_class=cond($css_class, $css_class)}