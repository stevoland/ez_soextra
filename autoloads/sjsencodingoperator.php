...
class sJSEncodingOperator
{
    ...
    {
		$this->Operators = array( 'js_encode_uri', 'js_decode_uri', 'js_encode_uri_component', 'js_decode_uri_component', 'js_escape', 'js_unescape' );
        $this->Debug = false;
    }
    ...
    function modify( &$tpl, &$operatorName, &$operatorParameters, &$rootNamespace, &$currentNamespace, &$operatorValue, &$namedParameters )
    {
        switch ( $operatorName )
        {
            case 'js_encode_uri':
            {
				$ret = $this->encodeURI($operatorValue);
            }break;
            case 'js_decode_uri':
            {
                $ret = $this->decodeURI($operatorValue);
            } break;
            case 'js_encode_uri_component':
            {
                $ret = $this->encodeURIComponent($operatorValue);
            }break;
            case 'js_decode_uri_component':
            {
                $ret = $this->decodeURIComponent($operatorValue);
            } break;
            case 'js_escape':
            {
                $ret = $this->encodeURIComponent($operatorValue);
    ...
    function encodeURI($string) {
       $result = "";
       for ($i = 0; $i < strlen($string); $i++) {
          $result .= $this->encodeURIbycharacter(urlencode($string[$i]));
       }
       return $result;
    }
    ...
    function decodeURI($string) {
       ...
       for ($i = 0; $i < strlen($string); $i++) {
           ...
           list($decodedstr, $num) = $this->decodeURIbycharacter($decstr);
           $result .= urldecode($decodedstr);
           $i += $num ;
       }
       return $result;
    ...
    function escape($string) {
       $result = "";
       for ($i = 0; $i < strlen($string); $i++) {
          $result .= $this->escapebycharacter(urlencode($string[$i]));
       }
       return $result;
    }
    ...
    function unescape($string) {
       ...
       for ($i = 0; $i < strlen($string); $i++) {
           ...
           list($decodedstr, $num) = $this->unescapebycharacter($decstr);
           $result .= urldecode($decodedstr);
           $i += $num ;
       }
       return $result;
