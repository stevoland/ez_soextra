...
tinyMCEPopup.onInit.add( eZOEPopupUtils.BIND( eZOEPopupUtils.init, window, {
    ...
    {        
		...
		var width_source = ez.$('embed_attr_width_source'),
			height_source = ez.$('embed_attr_height_source'),
			inline_width_source = ez.$('embed-inline_attr_width_source'),
			inline_height_source = ez.$('embed-inline_attr_height_source'),
			width = el ? el.getAttribute('width') || '' : def['width']  || '',
			height = el ? el.getAttribute('height') || '' : def['height']  || '';
		if ( width_source )
			width_source.el.value = width;
		if ( height_source )
			height_source.el.value = height;
		if ( inline_width_source )
			inline_width_source.el.value = width;
		if ( inline_height_source )
			inline_height_source.el.value = height;
...
function loadImageSize( e, el )
{
	...
	var sizes = ez.$('embed_attr_width_source', 'embed_attr_height_source', 'embed-inline_attr_width_source', 'embed-inline_attr_height_source');
