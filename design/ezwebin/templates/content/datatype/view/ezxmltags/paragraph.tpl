...
{def $style=''}
{if and(is_set( $align ), $align )}
	{if and(is_unset($#use_inline_styles), is_unset($use_inline_styles))}
		{set $classification=concat( $classification, ' text-', $align )}
	{else}
		{set $style=concat('text-align:',$align,'; ')}
	{/if}
{/if}{*
...
<p{if $classification|trim} class="{$classification|wash}"{/if}{if $style|ne('')} style="{$style}"{/if}>
