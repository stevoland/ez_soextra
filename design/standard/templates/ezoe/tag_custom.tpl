...
tinyMCEPopup.onInit.add( eZOEPopupUtils.BIND( eZOEPopupUtils.init, window, {
    ...
    onInitDone: function( el, tag, ed )
    {
        if ( !!el )
        {
            var align_source = jQuery('#' + customTagName + '_align_source'),
                width_source = jQuery('#' + customTagName + '_attr_width_source'),
                height_source = jQuery('#' + customTagName + '_attr_height_source'),
                width = el ? el.getAttribute('width') : '',
                height = el ? el.getAttribute('height') : '';
            if ( width_source )
                width_source.val(width);
            if ( height_source )
                height_source.val(height);
            if ( align_source && !!el.getAttribute('align') )
            {
                var align = '';
                if ( !!el.getAttribute('align') )
                    align = el.getAttribute('align') == 'middle' ? 'center' : el.getAttribute('align');
                align_source.val(align);
            }
    ...
    tagCreator: function( ed, tag, customTag, selectedHtml )
    {
        ...
        else if ( document.getElementById( customTag + '_inline_source' ).checked )
        {
            if ( document.getElementById( customTag + '_inline_source' ).value === 'image' )
            {
                ...
                // stevo
                var dimensions = '';
                var width_source = ez.$(customTag + '_attr_width_source'),
                    height_source = ez.$(customTag + '_attr_height_source'),
                    align_source = ez.$(customTag + '_align_source');
                
                if ( !!width_source && !!width_source.el.value )
                    dimensions += ' width="' + width_source.el.value + '"';
                if ( !!height_source && !!height_source.el.value )
                    dimensions += ' height="' + height_source.el.value + '"';
                    
                var align = '';
                if ( !!align_source )
                {
                    if ( align_source.el.value )
                    {
                        align = align_source.el.value == 'center' ? 'middle' : align_source.el.value;
                        align = ' align="' + align + '"';
                    }
                }
                
                return eZOEPopupUtils.insertHTMLCleanly( ed, '<img id="__mce_tmp" type="custom" src="' + imageSrc + '"' + dimensions + align + '" \/>', '__mce_tmp' );
            }
            else
    ...
    tagEditor: function( el, ed, customTag, args )
    {
        ...
        if ( target === 'img' )
        {
            ...
            if ( !!width_source )
                ed.dom.setAttrib( el, 'width', width_source.el.value || '' );
            if ( !!height_source )
                ed.dom.setAttrib( el, 'height', height_source.el.value || '' );
            if ( !!align_source )
            {
                var align = '';
                if ( align_source.el.value )
                    align = align_source.el.value == 'center' ? 'middle' : align_source.el.value
                ed.dom.setAttrib( el, 'align', align );
            }
        }
