...
tinyMCEPopup.onInit.add( ez.fn.bind( eZOEPopupUtils.init, window, {
    ...
    onInitDone: function( el, tag, ed )
    {
        if ( !!el )
        {
            var align_source = ez.$(customTagName + '_align_source'),
                width_source = ez.$(customTagName + '_attr_width_source'),
                height_source = ez.$(customTagName + '_attr_height_source'),
                width = el ? el.getAttribute('width') : '',
                height = el ? el.getAttribute('height') : '';
            if ( width_source )
                width_source.el.value = width;
            if ( height_source )
                height_source.el.value = height;
    ...
    tagGenerator: function( tag, customTag, selectedHtml )
    {
        ...
        else if ( document.getElementById( customTag + '_inline_source' ).checked )
        {
            if ( document.getElementById( customTag + '_inline_source' ).value === 'image' )
            {
                ...
                return '<img id="__mce_tmp" type="custom" src="' + imageSrc + '"' + dimensions + align +  '/>';
            }
            else
